...
2. [Предыстория](#предыстория)
3. [Скриншоты](#скриншоты)
4. [Возможности](#возможности)
5. [Установка прошивки](#установка-прошивки)
   - [Подготовка](#1-подготовка)
   - [Аппаратная подготовка](#2-аппаратная-подготовка-и-прошивка)
   - [Первое включение](#3-первое-включение)
6. [Архитектура](#архитектура-как-это-работает)
7. [Учёт электроэнергии](#функции-учёта-электроэнергии)
8. [Технические детали](#технические-детали)
9. [Сенсоры и сущности](#все-сенсоры-и-сущности)
10. [Сравнение](#зачем-это-сделано)
...
**Решил проблему кардинально:** заменил не только прошивку, но и сам модуль - переделал устройство с T1-3S на ESP8266 (ESP-12E), так как предыдущий модуль не поддерживает ESPHome.
...
| Защита и безопасность | Полный дашборд Home Assistant |
|:---:|:---:|
...
| Мониторинг 24/7 | Защита | Полный контроль |
|----------------|--------|-----------------|
| Напряжение, ток, мощность - с точностью до 0.1V и 1mA | Ток утечки (10-99 mA) | Веб-интерфейс ESPHome |
| 4 вида счётчиков энергии: общий, обратный, баланс, заряд | Перегруз по току (1-63A) | Управление через Home Assistant |
| Суточный/недельный/месячный учёт kWh | Перенапряжение / Пониженное напряжение | Кнопки сброса: день/неделя/месяц/все |
| Расчёт стоимости по тарифу (руб/кВт·ч) | Температурная защита (10-85°C) | Предоплатный режим (для аренды) |
| Синхронизация с внешним счётчиком | Автоматическое повторное включение | Сброс общей энергии через UART |
| - | Диагностика 20+ типов аварий | - |
...
### 2. Аппаратная подготовка и прошивка
